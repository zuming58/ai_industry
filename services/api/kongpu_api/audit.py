...
def audit_bundle(spec: dict[str, Any], bundle: GeneratedBundle) -> dict[str, Any]:
    ...
    for entity_type, id_key, items in entity_groups:
        ...
        for item in items:
            ...
            expected_entities.add((entity_type, entity_id))

    step_ids = {str(item.get("id")) for item in bundle.control_ir.get("steps", []) if item.get("id")}
    exception_ids = {str(item.get("exception_id")) for item in bundle.control_ir.get("exceptions", []) if item.get("exception_id")}
    signal_names_for_tests = {str(item.get("name")) for item in bundle.control_ir.get("signals", []) if item.get("name")}
    ...
    for test in bundle.test_spec.get("tests", []):
        ...
        elif source_step and str(source_step) not in step_ids:
        ...
        elif source_exception and str(source_exception) not in exception_ids:
        ...
        unknown = sorted(item for item in _expression_identifiers(expression) if item not in signal_names_for_tests)
        ...
        invalid_inputs = sorted(set((test.get("inputs") or {}).keys()) - external_input_names)
    ...
    for link in bundle.trace_links:
        ...
        trace_entities.add((entity_type, entity_id))
    ...
    for entity_type, entity_id in sorted(expected_entities - trace_entities):
    ...
    signal_ids = {str(item.get("id")) for item in signals if item.get("id")}
    ...
    for interlock in bundle.control_ir.get("interlocks", []):
        internal_symbols = sorted(
            ...
            if token not in known_interlock_symbols
        )
    ...
    for step in bundle.control_ir.get("steps", []):
        ...
        signal_directions = {str(item.get("name")): str(item.get("direction") or "").upper() for item in signals}
        for target in sorted(re.findall(r"([A-Za-z_][A-Za-z0-9_]*)\s*(?::=|=)", str(step.get("actions") or ""))):
            if signal_directions.get(target) not in {"DO", "AO", "INTERNAL", "COMM"}:
    ...
    input_hash = content_hash(stable_json({"generator_version": GENERATOR_VERSION, "files": bundle.files, "control_ir": bundle.control_ir, "test_spec": bundle.test_spec, "trace_links": bundle.trace_links}))
