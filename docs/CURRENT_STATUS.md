...
- P04 可导出只读 JSON/Markdown 校验报告，绑定原始 Excel SHA-256、当前 MachineSpec revision、PLC 目标、Template/Schema 和稳定排序的问题定位；导出不改变状态或验证等级。
...
- 本批次补齐本机备份恢复安全闭环：备份拒绝源目录符号链接并生成 `kongpu-local-backup/v1` 清单；恢复先校验路径、重复项、ZIP 体积、逐项 SHA-256 和 SQLite 完整性，再逐文件原子替换，损坏或篡改归档不会覆盖既有数据。仓库写入预算在锁内按现有内容与本次变更的投影结果重新计算，Git 读写及“检查后切分支”操作统一进入同一仓库锁。
...
| P04 导入校验 | 真实上传、规则、修订与 JSON/Markdown 校验报告导出 | 自动验证通过 |
...
- Playwright 加本机 Microsoft Edge：新建项目、下载范例、上传、P04 校验报告双格式下载、门禁、8 个视图确认、锁定、生成、加载后编辑、Commit、自动审核复用、编译证据、参考模拟、P09 候选 ZIP/独立复核/自动验收刷新恢复、P10 离线快照/调试分支、P11 双 Commit 结构化比较/统一时间线/恢复分支，以及 P12 设置持久化、兼容矩阵、模板历史、审计、只读 Adapter 检测和多视口溢出检查。
