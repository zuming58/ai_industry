...
- 本机备份使用 `kongpu-local-backup/v1` 清单：源目录遍历拒绝符号链接、非普通文件、Windows 保留名和读取期间的尺寸变化；归档限制条目数、单文件、压缩体积和总解压体积。恢复拒绝重复、加密、显式目录、路径穿越、清单不一致和超限归档，逐项 SHA-256 与 SQLite `PRAGMA quick_check` 预检通过后才原子替换；旧的无清单备份直接拒绝。
...
`inovance-h5u-st-v1` 使用 AutoShop 作为目标 Adapter，但当前 AutoShop 实现仍是只读检测加 `manual_required` 降级，不启动或调用未知程序。H5U 的逻辑地址、ST 方言细节、AutoShop 工程格式、编译诊断和厂商模拟必须在集中验证包中用精确软件版本和真实设备确认；本机参考模拟不替代这些验证。
...
必须自动覆盖：环境变量命令注入、路径穿越、符号链接逃逸、超大文件、Git 超时与 stdout/stderr 合并流式上限、恶意文件名、现有仓库投影预算、脏工作树切分支、备份清单篡改、损坏 SQLite、日志脱敏、工件哈希/大小不一致、旧 Commit/旧 TestSpec 结果复用和并发 409。任何失败都只能阻断任务并保留证据，不能静默修复或覆盖基线。
