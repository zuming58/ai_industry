...
用户当前没有 PLC 硬件和持续电气工程师支持，也不承担代码审查或日常调试决策。开发按以下顺序进行：

1. Agent 自主完成不依赖硬件的调研、实现、静态检查、单元/集成/E2E、隔离夹具、失败路径和安全审计；
2. 对官方文档、标准和开源候选记录来源、许可证、维护状态、适用范围与未验证边界；
3. 使用保守默认值继续开发，所有假设与验证方法写入状态文档；
4. 黄金资料到位后先由 Agent 自动导入、比对和生成差异，不要求用户做代码级判断；
5. 只有厂商工具、真实 PLC、现场接线或电气专业判断不可替代时，统一进入 1–2 天集中验证；
6. 验证发现必须转成自动化回归用例，减少再次依赖人工。

集中验证所需的黄金项目、软件、硬件、步骤、证据和回退见 docs/ELECTRICAL_VALIDATION_PACKAGE.md。M1/M2 在此之前只能标记“自动验证通过”，不能标记厂商工具、硬件或电气确认通过。
