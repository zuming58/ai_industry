...
function ImportPage() {
  ...
  return <ProjectPage kicker="P04 · IMPORT" title="Excel 导入与校验" description="原文件不可变保存，页面修订会创建新的 MachineSpec revision。" actions={<><input ref={inputRef} type="file" hidden accept=".xlsx" onChange={(event) => upload(event.target.files?.[0])} />{spec && <><button className="button button--outline" disabled={Boolean(exporting)} onClick={() => downloadReport("json")}><DownloadSimple size={16} />校验报告 JSON</button><button className="button button--outline" disabled={Boolean(exporting)} onClick={() => downloadReport("markdown")}><DownloadSimple size={16} />校验报告 Markdown</button><button className="button button--outline" disabled={Boolean(exporting)} onClick={() => downloadReport("xlsx")}><DownloadSimple size={16} />下载标记 Excel</button></>}<button className="button button--primary" disabled={uploading} onClick={() => inputRef.current?.click()}><UploadSimple size={16} />{uploading ? "校验中…" : "上传 XLSX"}</button></>}>
