...
function ImportPage() {
  const queryClient = useQueryClient(); const { projectId, project } = useProjectContext(); const inputRef = useRef<HTMLInputElement>(null); const [uploading, setUploading] = useState(false); const [exporting, setExporting] = useState<string | null>(null); const [selectedIssue, setSelectedIssue] = useState<ValidationIssue | null>(null); const [editValue, setEditValue] = useState("");
  ...
  const downloadReport = async (kind: "json" | "markdown" | "xlsx") => { if (!imported.data) return; setExporting(kind); try { const blob = await api.downloadImportValidationReport(imported.data.id, kind); const code = project.data?.code || projectId.slice(0, 8); const extension = kind === "json" ? "json" : kind === "markdown" ? "md" : "xlsx"; saveBlob(blob, `${code}-validation-report.${extension}`); notify(kind === "json" ? "校验报告 JSON 已下载" : kind === "markdown" ? "校验报告 Markdown 已下载" : "标记 Excel 副本已下载"); } catch (error) { notifyError(error); } finally { setExporting(null); } };
  const downloadOriginal = async () => { if (!imported.data?.source_artifact_id) return; setExporting("original"); try { const blob = await api.downloadArtifact(imported.data.source_artifact_id); saveBlob(blob, imported.data.filename || "MachineSpec-original.xlsx"); notify("原始 Excel 已下载；原件保持不可变"); } catch (error) { notifyError(error); } finally { setExporting(null); } };
  return <ProjectPage kicker="P04 · IMPORT" title="Excel 导入与校验" description="原文件不可变保存，页面修订会创建新的 MachineSpec revision。" actions={<><input ref={inputRef} type="file" hidden accept=".xlsx" onChange={(event) => upload(event.target.files?.[0])} />{spec && <><button className="button button--outline" disabled={Boolean(exporting)} onClick={downloadOriginal}><DownloadSimple size={16} />下载原始 Excel</button><button className="button button--outline" disabled={Boolean(exporting)} onClick={() => downloadReport("json")}><DownloadSimple size={16} />校验报告 JSON</button><button className="button button--outline" disabled={Boolean(exporting)} onClick={() => downloadReport("markdown")}><DownloadSimple size={16} />校验报告 Markdown</button><button className="button button--outline" disabled={Boolean(exporting)} onClick={() => downloadReport("xlsx")}><DownloadSimple size={16} />下载标记 Excel</button></>}<button className="button button--primary" disabled={uploading} onClick={() => inputRef.current?.click()}><UploadSimple size={16} />{uploading ? "校验中…" : "上传 XLSX"}</button></>}>
...
function ProjectTimelineView({ projectId, timeline }: { projectId: string; timeline?: { events: ProjectTimelineEvent[]; summary: { total: number; by_type: Record<string, number> }; claim_boundary: string } }) {
  const events = timeline?.events || [];
  const [busy, setBusy] = useState<string | null>(null);
  const download = async (kind: "json" | "markdown") => {
    setBusy(kind);
    try {
      const blob = await api.downloadProjectTimeline(projectId, kind);
      saveBlob(blob, `Kongpu-${projectId.slice(0, 8)}-timeline.${kind === "json" ? "json" : "md"}`);
      notify(kind === "json" ? "项目时间线 JSON 已下载" : "项目时间线 Markdown 已下载");
    } catch (error) {
      notifyError(error);
    } finally {
      setBusy(null);
...
function ReleasePage() {
  ...
  const downloadEvidenceLedger = async (item: ReleaseCandidate, kind: "json" | "markdown") => {
    try {
      const blob = await api.downloadEvidenceLedger(item.id, kind);
      saveBlob(blob, "Kongpu-" + item.version + (kind === "json" ? "-evidence-ledger.json" : "-evidence-ledger.md"));
      notify(kind === "json" ? "证据台账 JSON 已下载" : "证据台账 Markdown 已下载");
    } catch (error) {
      notifyError(error);
    }
  };
...
function DeviceLibrary() {
  const compatibility = useQuery({ queryKey: ["compatibility-matrix"], queryFn: api.getCompatibilityMatrix });
  const templates = [{ name: "双电控气缸", detail: "状态机、伸出/缩回反馈、超时与互锁", status: "确定性骨架生成" }, { name: "简单伺服/变频器握手", detail: "使能、就绪、到位与故障握手", status: "确定性骨架生成" }, { name: "自动/手动模式", detail: "模式切换、复位路径和受控工步监控", status: "确定性骨架生成" }, { name: "安全 PLC", detail: "急停、门锁和安全回路不自动生成", status: "不自动生成" }];
  return <main className="hub-page"><PageHeading kicker="DEVICE LIBRARY" title="设备库" description="真实 PLC Profile 与已支持的控制模板目录；厂商编译和硬件能力仍单独标记为未验证。" /><section className="asset-section"><div className="asset-section__heading"><div><h3>PLC 目标 Profile</h3><p>来源：本机兼容矩阵 API</p></div><Status value={compatibility.isLoading ? "读取中" : `${compatibility.data?.entries.length || 0} 个型号`} /></div><section className="asset-grid">{compatibility.data?.entries.map((entry) => <article className="panel asset-card-static" key={entry.target.model}><Cpu size={24} /><div><strong>{entry.target.model}</strong><small>{entry.target.brand} · {entry.target.series} · {entry.vendor_tool}</small><small>ST 生成：{entry.structured_text_generation} · 参考模拟：{entry.reference_simulation}</small></div><Status value={entry.vendor_compile === "unverified" ? "厂商未验证" : entry.vendor_compile} /></article>)}{compatibility.isLoading && <EmptyState title="正在读取 PLC Profile" text="连接本机 API…" />}{compatibility.isError && <EmptyState title="兼容矩阵读取失败" text={errorMessage(compatibility.error)} />}</section></section><section className="asset-section"><div className="asset-section__heading"><div><h3>控制模板</h3><p>生成器可自动产出骨架，具体工程仍需验证</p></div><Status value="M2 支持" /></div><section className="asset-grid">{templates.map((device) => <article className="panel asset-card-static" key={device.name}><Cube size={24} /><div><strong>{device.name}</strong><small>{device.detail}</small></div><Status value={device.status} /></article>)}</section></section><div className="p12-claim"><Info size={15} />{compatibility.data?.claim_boundary || "兼容矩阵仅描述代码和参考模拟能力，不代表厂商工具、真实 PLC 或安全确认。"}</div></main>;
}

function DocumentsPage() {
  const projects = useProjects(); const activeProject = projects.data?.find((item) => !item.archived);
  const docs = [{ name: "MachineSpec 模板中心", type: "Excel 契约", location: "P03 模板与范例", href: activeProject ? `/projects/${activeProject.id}/templates` : "/projects" }, { name: "Excel 导入与确定性校验", type: "真实工作区", location: "P04 导入校验", href: activeProject ? `/projects/${activeProject.id}/imports` : "/projects" }, { name: "MachineSpec JSON Schema v1", type: "JSON Schema", location: "/api/v1/schemas/machine-spec/v1", href: "/api/v1/schemas/machine-spec/v1" }, { name: "本机 API 文档", type: "OpenAPI", location: "FastAPI Swagger", href: "http://127.0.0.1:8000/docs", external: true }, { name: "开发状态与边界", type: "Markdown", location: "docs/CURRENT_STATUS.md", href: "https://github.com/zuming58/ai_industry/blob/main/docs/CURRENT_STATUS.md", external: true }];
  return <main className="hub-page"><PageHeading kicker="DOCUMENTS" title="文档资料" description="产品契约、API、Excel 工作区和开发边界的真实入口。" /><section className="panel document-panel"><div className="document-list__head"><span>名称</span><span>类型</span><span>位置</span><span>打开</span></div>{docs.map((doc) => <div className="document-row-real" key={doc.name}><span><FileText size={17} /><strong>{doc.name}</strong></span><span>{doc.type}</span><code>{doc.location}</code><a href={doc.href} target={doc.external ? "_blank" : undefined} rel={doc.external ? "noreferrer" : undefined} aria-label={`打开 ${doc.name}`}>打开 <ArrowRight size={14} /></a></div>)}{projects.isLoading && <EmptyState title="正在读取项目入口" text="连接本机 API…" />}</section></main>;
}
