...
import type {
  GenerationRun, MachineSpec, ProgramBranch, ProgramCommit, ProgramFile,
  Project, SpecRevision, ValidationIssue,
} from "./domain";
...
function AppSidebar() {
  ...
  })}</nav><div className="sidebar__bottom"><Link className={"nav-item " + (location.pathname === "/settings" ? "is-active" : "")} to="/settings"><span className="nav-item__icon"><GearSix size={23} /></span><span>系统设置</span></Link><div className="sidebar__version">KONGPU · M2</div></div></aside>;
}
...
function AppShell({ toast, clearToast }: { toast: ToastState; clearToast: () => void }) {
  return <div className="app-shell"><AppSidebar /><div className="app-frame"><Topbar /><Routes><Route path="/" element={<Dashboard />} /><Route path="/projects" element={<ProjectsPage />} /><Route path="/specs" element={<ProjectChooser destination="review" title="规格管理" />} /><Route path="/program" element={<ProjectChooser destination="program" title="程序工程" />} /><Route path="/versions" element={<ProjectChooser destination="versions" title="版本控制" />} /><Route path="/debug" element={<ProjectChooser destination="compile" title="调试工具" />} /><Route path="/devices" element={<DeviceLibrary />} /><Route path="/documents" element={<DocumentsPage />} /><Route path="/settings" element={<UnavailablePage title="系统设置" detail="厂商工具、模型和运行环境配置将在后续阶段接入。" />} /><Route path="/projects/:projectId/templates" element={<TemplatePage />} /><Route path="/projects/:projectId/imports" element={<ImportPage />} /><Route path="/projects/:projectId/review" element={<ReviewPage />} /><Route path="/projects/:projectId/program" element={<ProgramPage />} /><Route path="/projects/:projectId/versions" element={<VersionPage />} /><Route path="/projects/:projectId/:capability" element={<CapabilityPage />} /><Route path="*" element={<Navigate to="/" replace />} /></Routes><div className="statusbar"><span><Database size={13} /> SQLite WAL · 本机数据</span><span>FX5U · Structured Text · M1/M2</span></div></div>{toast && <div className={"toast " + (toast.tone === "error" ? "toast--error" : "")}><CheckCircle size={19} /><span>{toast.message}</span><button onClick={clearToast}><X size={15} /></button></div>}</div>;
}
...
function CapabilityPage() {
  const { capability = "" } = useParams(); const labels: Record<string, string> = { compile: "P07 程序编译", simulation: "P08 自动模拟", release: "P09 发布包", monitor: "P10 在线监控" };
  return <ProjectPage kicker="M3 / M4 BOUNDARY" title={labels[capability] || capability} description="该页面尚未接入真实厂商能力。"><UnavailableBody detail={capability === "compile" ? "当前电脑未检测到 GX Works3；M2 生成的 ST 骨架尚未经过厂商编译。" : capability === "simulation" ? "当前电脑未检测到 GX Simulator3，不会伪造模拟通过结果。" : capability === "monitor" ? "未实现 PLC 下载、RUN/STOP、强制输出或真实在线连接。" : "发布能力将在真实编译、模拟和安全审查之后接入。"} /></ProjectPage>;
}
