...
test("P01-P06 and P11 complete the real local workflow", async ({ page }) => {
  ...
  const originalWorkbookDownload = page.waitForEvent("download");
  await page.getByRole("button", { name: "下载原始 Excel" }).click();
  expect((await originalWorkbookDownload).suggestedFilename()).toBe("MachineSpec_example.xlsx");
  const importValidationJsonDownload = page.waitForEvent("download");
  await page.getByRole("button", { name: "校验报告 JSON" }).click();
  expect((await importValidationJsonDownload).suggestedFilename()).toContain("validation-report.json");
  ...
  await page.getByRole("button", { name: "项目时间线" }).click();
  await expect(page.getByRole("heading", { name: "项目时间线" })).toBeVisible();
  await expect(page.locator(".project-timeline__list")).toContainText("MachineSpec Revision");
  await expect(page.locator(".project-timeline__list")).toContainText("项目自动审核");
  await expect(page.locator(".project-timeline__list")).toContainText("控谱参考逻辑模拟");
  await expect(page.locator(".project-timeline__list")).toContainText("交付候选包 RC-0001");
  await expect(page.locator(".project-timeline__list")).toContainText("项目自动验收");
